...
public class Super_classDAO {
    ...
    private final String insertExpr = "INSERT INTO super_class(id_super_class,code,description) VALUES(nextval(('super_class_s'::text)::regclass),?,?)";
    private final String updateExpr = "UPDATE super_class SET code=?,description=? WHERE id_super_class=?";
    private final String removeExpr = "DELETE FROM super_class WHERE id_super_class_id=?";
    private final String selectExpr = "SELECT id_super_class FROM super_class WHERE code=?";
    private final String selectByPKeyExpr = "SELECT id_super_class,code,description FROM super_class WHERE id_super_class=?";
    private final String selectByIdxExpr = "SELECT id_super_class,code,description FROM super_class WHERE code=?";

    public Super_classDAO() {
    ...
    public boolean insert(Super_class record) {
        errorMsg = "";
        try {
    ...
    public boolean update(Super_class record) {
        errorMsg = "";
        try {
            updateStmt.setString(1, record.getCode());
            updateStmt.setString(2, record.getDescription());
            updateStmt.setInt(3, record.getId_super_class());
    ...
    public boolean insertOrUpdate(Super_class record) {
        errorMsg = "";
        try {
            selectStmt.setString(1, record.getCode());
            ResultSet rs = selectStmt.executeQuery();
            if (rs.next()) {
                record.setId_super_class(rs.getInt("id_super_class"));
                rs.close();
    ...
    public boolean remove(Super_class record) {
        errorMsg = "";
        try {
            removeStmt.setInt(1, record.getId_super_class());
    ...
    public boolean loadByPKey(Super_class record) {
        errorMsg = "";
        try {
            boolean found = false;
            selectByPKeyStmt.setInt(1, record.getId_super_class());
    ...
    public boolean loadByIdx(Super_class record) {
        errorMsg = "";
        try {
            boolean found = false;
    ...
    public List<Super_class> loadRecordList(String whereExpr, List<JdbcParam> params) {
        errorMsg = "";
        try {
            List<Super_class> records = new ArrayList<>();
            String selectByWhereExpr = "SELECT id_super_class,code,description FROM super_class " + whereExpr;
            ConnectioManager cm = ConnectioManager.getInstance();
            try (PreparedStatement selectByWhereStmt = cm.createPreparedStatement(selectByWhereExpr)) {
                ...
                try (ResultSet rs = selectByWhereStmt.executeQuery()) {
                    Super_class record = null;
                    while (rs.next()) {
                        record = new Super_class();
                        record.setId_super_class(rs.getInt("id_super_class"));
                        record.setCode(rs.getString("code"));
                        record.setDescription(rs.getString("description"));
                        records.add(record);
