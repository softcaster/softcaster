...
public class DaycountDAO {
    ...
    private final String insertExpr = "INSERT INTO daycount(id_daycount,code,description) VALUES(nextval(('daycount_s'::text)::regclass),?,?)";
    private final String updateExpr = "UPDATE daycount SET code=?,description=? WHERE id_daycount=?";
    private final String removeExpr = "DELETE FROM daycount WHERE id_daycount_id=?";
    private final String selectExpr = "SELECT id_daycount FROM daycount WHERE code=?";
    private final String selectByPKeyExpr = "SELECT id_daycount,code,description FROM daycount WHERE id_daycount=?";
    private final String selectByIdxExpr = "SELECT id_daycount,code,description FROM daycount WHERE code=?";
    ...
    public boolean insert(Daycount record) {
        errorMsg = "";
        try {
    ...
    public boolean insertOrUpdate(Daycount record) {
        errorMsg = "";
        try {
            selectStmt.setString(1, record.getCode());
            ResultSet rs = selectStmt.executeQuery();
            if (rs.next()) {
                record.setId_daycount(rs.getInt("id_daycount"));
                rs.close();
    ...
    public boolean remove(Daycount record) {
        errorMsg = "";
        try {
            removeStmt.setInt(1, record.getId_daycount());
    ...
    public boolean loadByPKey(Daycount record) {
        errorMsg = "";
        try {
            boolean found = false;
            selectByPKeyStmt.setInt(1, record.getId_daycount());
    ...
    public boolean loadByIdx(Daycount record) {
        errorMsg = "";
        try {
            boolean found = false;
    ...
    public List<Daycount> loadRecordList(String whereExpr, List<JdbcParam> params) {
        errorMsg = "";
        try {
            List<Daycount> records = new ArrayList<>();
            String selectByWhereExpr = "SELECT id_daycount,code,description FROM daycount " + whereExpr;
            ConnectioManager cm = ConnectioManager.getInstance();
