...
//        System.out.println(CashFlowUtil.asString(cf));        
    ...
    public static double yield(final Date settlement, final Date maturity, final double couponRate, final double price, final int frequency, final DayCountBasis basis) {
        Function1 f = new Function1() {            
            ...
            public double f(double x) {     
                double calcPrice = BondValuation.price(settlement, maturity, couponRate, x, frequency, basis);
        ...
        return MathUtil.rootNewton(f, 0.1,Compounding.COMPOUNDED);        
    }
    ...
    public static double[] yield(final Date[] settlement, final Date[] maturity, final double[] couponRate, final double[] price, final int[] frequency, final DayCountBasis[] basis) {
        ...
        for (int i = 0; i < len; i++) {
            yields[i] = BondValuation.yield(settlement[i], maturity[i], couponRate[i], price[i], frequency[i], basis[i]);
        }
