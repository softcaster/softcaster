 ...
 *
 * We could have gotten away with a lot of things by using JodaTime, but the
 * goal is to have no dependency.
 *
 * @author jon
 *
 */
public final class DateUtil {
    ...
    /**
     * Actual days between dates. Convert first to Julian date (number) then
     * subtract. Used formula from TIPS.
     *
    ...
    public static Date addMonths30NASD(Date date, int months) {
        ...
        if (month > 12) {
            int nyears = (month / 12);
            if (month % 12 == 0) {
                nyears -= 1;  // month==12 (December) is ok
            }
            year += nyears;
            month -= 12 * nyears;
        } else if (month < 1) {
