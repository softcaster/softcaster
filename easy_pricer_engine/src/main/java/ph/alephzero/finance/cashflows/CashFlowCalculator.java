...
/**
 * Time value of money calculations (PV, PMT, FV) return the same sign as the
 * input cash flows (unlike the reverse sign convention e.g. in Excel, HP 130c).
 *
...
public final class CashFlowCalculator {
     ...
     * @param arrears annuity payment in arrears (TRUE) or at the beginning of
     * the period (FALSE)
     ...
     */
    public static double presentValue(CashFlows cashflows, double rate, Compounding compounding) {
        double value = 0.0;
     ...
     * @param arrears payment in arrears (TRUE) or at the beginning of the
     * period (FALSE)
     ...
     * @param cashflows non-dated cash flow
     * @param rate rate to be used for discounting
     * @param periods number of periods. If -1 then use cashflows.getCount() -
     * 1.
     *
     ...
     * @param arrears payment in arrears (TRUE) or at the beginning of the
     * period (FALSE)
     * @return equal payment amount equivalent of cash flows represented by the
     * input
     */
    ...
    public static double annuities(CashFlows cashflows, double rate, int periods, boolean arrears) {
        double pv = presentValue(cashflows, rate, Compounding.COMPOUNDED);
    ...
    public static double internalRateOfReturn(int periods, double annuity, double presentValue, double futureValue, boolean arrears) {
        ...
        return MathUtil.rootNewton(f, 0.10, Compounding.COMPOUNDED);
    }
     ...
     * @return
     */
    public static double internalRateOfReturn(CashFlows cashflows, Compounding compounding) {
        final CashFlows _cashflows = cashflows;

        MathUtil.Function1 f = new MathUtil.Function1() {
            @Override
            public double f(double rate) {
                return presentValue(_cashflows, rate, Compounding.COMPOUNDED);
            }

            @Override
            public double f(double x, Compounding compounding) {
                return presentValue(_cashflows, x, compounding);
            }
        };
        return MathUtil.rootNewton(f, 0.10, compounding);
    }
