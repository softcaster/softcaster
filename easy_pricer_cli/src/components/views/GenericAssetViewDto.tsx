...
import { useFinancialViewDto } from '../hooks/useFinancialViewDto'; // L'hook creato prima

interface GenericAssetViewDtoProps<TMasterDto> {
    assetClass: string;
    fetchMasterData: () => Promise<TMasterDto[]>;
    defaultTxn: any;
...
export function GenericAssetViewDto<TMasterDto>({
    assetClass,
    fetchMasterData,
    defaultTxn,
    FormComponent,
    TableComponent
}: GenericAssetViewDtoProps<TMasterDto>) {

    const {
        masterDataList, positionList, counterpartyList,
        trades, selectedTrade, setSelectedTrade
    } = useFinancialViewDto(assetClass, fetchMasterData, defaultTxn);
